...
public class ChestShopGUI extends InventoryGUI {
	...
	public ChestShopGUI(int id, MySQL mysql, MChestShop plugin, String desc, ItemStack item, double price, boolean ankauf, Location chestLoc, String owner, String economy, Location loc) {
		super(plugin, desc, 3);
		this.id = id;
		this.plugin = plugin;
		this.desc = desc;
	...
	public void click(Player paramPlayer, int paramInt, boolean paramBoolean) {
		...
		if(paramInt == 10) {
			...
			double kontostand = !ankauf ? MoneyAPI.get(paramPlayer.getUniqueId()) : MoneyAPI.get(UUID.fromString(owner));
